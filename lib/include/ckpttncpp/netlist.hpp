      ...
      num_nets(nets.size())
{
    ...
    auto result1         = std::max_element(this->modules.begin(), this->modules.end(), deg_cmp);
    this->max_degree     = this->G.degree(*result1);
    auto result2         = std::max_element(this->nets.begin(), this->nets.end(), deg_cmp);
    this->max_net_degree = this->G.degree(*result2);
}

template<typename nodeview_t, typename nodemap_t>
Netlist<nodeview_t, nodemap_t>::Netlist(graph_t&& G, size_t numModules, size_t numNets)
    : Netlist{std::forward<graph_t>(G), py::range<int>(numModules),
              py::range<int>(numModules, numModules + numNets), py::range<int>(numModules),
