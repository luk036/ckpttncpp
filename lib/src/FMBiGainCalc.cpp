    ...
    node_t net, const std::vector<uint8_t>& part)
{
    ...
    if (part[i_w] != part[i_v]) { this->totalcost += weight; }
    else
    {
        weight = -weight;
    }
...
void FMBiGainCalc::init_gain_general_net(node_t net, const std::vector<uint8_t>& part)
{
    ...
    if (num[0] > 0 && num[1] > 0) { this->totalcost += weight; }
 ...
 */
auto FMBiGainCalc::update_move_2pin_net(const std::vector<uint8_t>& part, const MoveInfo& move_info)
    -> ret_2pin_info
{
    ...
    -> ret_info
{
    ...
    if (part_w != fromPart) { weight = -weight; }
