                         ...
                         const py::set<node_t>& DontSelect)
{
    ...
    while (!bpq.is_empty())
    {
        ...
        if (visited[i_net]) { continue; }
        auto net = H.nets[i_net];
        if (H.G.degree(net) < 2) { continue; }
        S.insert(net);
...
auto create_contraction_subgraph(SimpleNetlist& H, const py::set<node_t>& DontSelect)
{
    ...
    auto H2 = std::make_unique<SimpleNetlist>(
        std::move(G), py::range<int>(numModules), py::range<int>(numModules, num_vertices),
